"""Trace contact with NO marker offsets."""
import sys, os
...
def load_quad(path):
    ...
    for t in tris:
        v=verts[t]; X_q.append((v[0]+v[1]+v[2])/3); w_q.append(0.5*np.linalg.norm(np.cross(v[1]-v[0],v[2]-v[0])))
...
cube_sdf=TrilinearSDFGrid(os.path.join(SC,'models','rmd_box_res128.sdf'))
...
for step in range(150):
    ...
    if step>=130 or m.active_count>0:
        print(f'Step{step:3d}: t={state.t:.4f} z={state.r[idx4,2]:.5f} vz={state.v[idx4,2]:.4f} gap={m.gap*1e6:.0f}um act={m.active_count} Fz={Q[6*1+2]:.0f}N ph={ph}')
    intg.step(state,0.001)
    if state.r[idx4,2]<-0.05:
        print(f'Step{step:3d}: t={state.t:.4f} z={state.r[idx4,2]:.4f} vz={state.v[idx4,2]:.4f} PHẦN TỬ!')
        break
