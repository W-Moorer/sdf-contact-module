...
class QuadratureMesh:
    def __init__(self, X_q, w_q, tri_normals=None, tri_ids=None):
        self.X_q = np.asarray(X_q, dtype=float)
        self.w_q = np.asarray(w_q, dtype=float)
        self.tri_normals = tri_normals
        self.tri_ids = tri_ids
        self.num_points = len(X_q)

    def transform(self, r, R):
        X_w = r + R @ self.X_q.T
        return X_w.T
...
def triangle_centroid_quadrature(verts, tris):
    ...
    return X_q, areas, tri_normals, np.arange(len(tris))


def quadrature_mesh_from_obj(path):
    verts, tris = load_obj_triangles(path)
    X_q, w_q, tri_normals, tri_ids = triangle_centroid_quadrature(verts, tris)
    return QuadratureMesh(X_q, w_q, tri_normals, tri_ids)


def select_bottom_face(X_q, tri_normals, tol=0.01):
    return tri_normals[:, 2] < -(1.0 - tol)
